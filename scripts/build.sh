#!/usr/bin/env bash
# Build Tablor for Ableton Move (aarch64) — from the Mac.
#
# The Mac has no toolchain and no Docker (deliberately). This script:
#   1. rsyncs the source up to the VPS (~/schwung-dev/schwung-tablor)
...
VPS="vps"
REMOTE_DIR="schwung-dev/schwung-tablor"
TARGET="${1:-all}"
...
rsync -az --delete \
    --exclude .git --exclude reference/ --exclude build/ --exclude build-native/ \
    --exclude dist/ --exclude .DS_Store \
    "$SRC/" "$VPS:$REMOTE_DIR/"

echo "=== 2/3 docker build on $VPS"
ssh "$VPS" "cd $REMOTE_DIR && \
    docker image inspect schwung-tablor-builder >/dev/null 2>&1 || \
        docker build -t schwung-tablor-builder -f scripts/Dockerfile scripts/ && \
    docker run --rm -v \$PWD:/build -u \$(id -u):\$(id -g) -w /build \
        schwung-tablor-builder ./scripts/docker-build.sh $TARGET"
...
echo; echo "=== Artifacts ==="
ls -la "$SRC/build/" | grep -E "\.so|loadtest|bench" || true
